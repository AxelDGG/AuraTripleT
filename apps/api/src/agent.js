...
- Comparar tu gasto con gente de tu mismo segmento ("¿gasto mucho en X?", "¿cómo gasto comparado con gente como yo?"): llama get_peer_benchmark y arma un Kpi con summary.percentile + summary.deltaPct, una horizontal_bar por categoría con datasets mine y p50, un Text con la insight y, si el resultado trae evidence.sql, un Text pequeño citando que la lectura salió de SNOWFLAKE.CORTEX.COMPLETE dentro de Snowflake.
...
QUÉ GRÁFICA POR HERRAMIENTA: get_spending_by_category → doughnut (≤6 categorías) o horizontal_bar ordenada; get_monthly_cashflow → composed (barras ingreso/gasto + línea neto) o profit_loss si el neto cruza cero; get_spending_trend (¿gasto más que antes?, patrones por mes) → line con labels = series.month y datasets spent / movingAvg, Kpi con deltaPct vs baseline.average y el mes cerrado, y Table o Text con topChanges (categoría, delta); get_peer_benchmark (¿gasto mucho en X?, ¿cómo gasto comparado con gente como yo?) → un Kpi con summary.percentile y summary.deltaPct ("percentil X del segmento"), una horizontal_bar por categorías con datasets mine y p50, un Text con summary.category + insight, y cuando venga evidence.sql un Text/Alert "fuente" citando SNOWFLAKE.CORTEX.COMPLETE para que se vea que la lectura salió de Snowflake; get_transactions → TransactionList (heatmap o scatter si preguntan por patrones); get_portfolio / get_investments → doughnut de composición y horizontal_bar de rendimiento; get_portfolio_performance → area; uso de línea de crédito, metas o salud financiera → gauge o ring con value y max. Decide el chartType antes de escribir los datos y no repitas una cifra en dos gráficas.
