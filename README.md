...
| **Tiger Data** | App rápida y escalable con SQL | **Base operacional y memoria del agente — integrado.** Postgres 18 + TimescaleDB 2.30 en Tiger Cloud. Dos hypertables: `transactions` (movimientos, chunks de 30 días) y `ui_history` (cada interfaz que genera el agente, con la carpeta que él mismo eligió, chunks de 7 días e índice trigram sobre el título para el buscador). **Continuous aggregates** `spending_by_category_monthly` y `monthly_cashflow` con refresh policy y agregación en tiempo real: `get_spending_by_category`, `get_monthly_cashflow` y `get_spending_trend` leen de ahí, no vuelven a recorrer los movimientos. **Toolkit**: `get_spending_trend` rellena los meses con `time_bucket_gapfill` y resume la línea base con `stats_agg` → `average`/`stddev` ("en agosto gastaste 28% más que tu promedio; Compras es lo que más subió"). **Memoria del agente**: tabla `customer_memory` con `pgvector` (embeddings Gemini de 768 dims, índice HNSW): al cerrar cada turno un LLM extrae hechos duraderos ("prefiere plazos cortos", "Ana es su casera") y en el siguiente se recuperan los más parecidos a la pregunta y van al prompt. Las tools MCP leen vía repositorios (`memory` \| `tiger`, se elige con `BANK_DATA_SOURCE`); la transferencia es atómica con la condición de saldo dentro del `UPDATE`. Carga con `npm run db:setup`, paridad contra la base real con `npm run db:verify`. Local: contenedor TimescaleDB en `infra/docker-compose.yml`. | Cuentas, movimientos, gastos, tendencia, historial de visualizaciones y memoria leyendo y escribiendo en Tiger |
| **Snowflake** | Uso de Snowflake para APIs y LLMs | **Capa analítica — integrado.** Dataset sintético poblacional (miles de clientes, 24 meses) generado de forma determinista (`npm run peer:seed` → `data/snowflake/*.csv`) para **benchmarks de pares** ("gastas 30% más que personas como tú en restaurantes"). **Tool MCP `get_peer_benchmark`**: percentiles por categoría (p25/p50/p75/p90) contra tu segmento + la lectura en lenguaje natural. Se sirve **por la SQL REST API** (`POST /api/v2/statements`, auth Basic, polling de handle — el mismo endpoint del ejemplo del reto) y las consultas van a **Cortex**: `SNOWFLAKE.CORTEX.COMPLETE` genera la insight (el agente muestra la cita SQL en la UI), con `AI_AGG`/`AI_CLASSIFY` documentados en `data/snowflake/001_peer_schema.sql` como próximos pasos. **Fallback**: sin `SNOWFLAKE_*` el benchmark se calcula con la población local (mismos datos, misma semilla); `BANK_DATA_SOURCE=snowflake` delega lo operativo al repositorio local. | Una consulta Cortex visible dentro de la UI generada |
...
| `SNOWFLAKE_ACCOUNT`, `SNOWFLAKE_USER`, `SNOWFLAKE_ACCESS_TOKEN`, `SNOWFLAKE_WAREHOUSE`, `SNOWFLAKE_DATABASE` | Hoy (opcional) | Benchmarks de pares y Cortex via SQL REST API (`/api/v2/statements`). Los trials nuevos exigen PAT (`ALTER USER … ADD PROGRAMMATIC ACCESS TOKEN`) o key-pair JWT, no contraseña. Sin ellas `get_peer_benchmark` usa la población sintética local (`data/snowflake/`) |
